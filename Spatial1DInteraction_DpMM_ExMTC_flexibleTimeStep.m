function [Ne, Cmp, Ne0, Cmp0, dist, AllCmp] = Spatial1DInteraction_DpMM_ExMTC_flexibleTimeStep(Nr,r0,SpPopDist,rint,TID,A,B,kSat,kY,ExtTh,DilTh,tauf,dtau,Nz,Z,DCell,DMed,dt,dist,dc)
...
nCell = TID * Nz * SpPopDist; % initial number of each cell type
nCellt = zeros(Nc,Nt);
cMedt = zeros(Nm,Nt);
crt = 0;
for iRound = 1:Nr
    ...
    dtauStep = 0;
    reffStep = 0;
    %for timestep = 0:(min([dt,dtau,1/reff])/3):tauf
    while timestep <= tauf && sum(sum(nCell)) < (DilTh * Nz)
        timestep = timestep + min([dt,dtau,1/reff]);
         if timestep-dtStep >= dt
             realDt = timestep-dtStep;
             fMedm = realDt*DMed/(dz^2)*[cMed(1,:); cMed(1:Nz-1,:)];
             fMedp = realDt*DMed/(dz^2)*[cMed(2:Nz,:); cMed(Nz,:)];
             fMedc = -2*realDt*DMed/(dz^2)*cMed;
             cMedCons = realDt*nCell*A';
             cMed = cMed + realDt*nCell*B' - cMedCons + (fMedm+fMedc+fMedp);
             cMed = cMed.*(cMed>0);
             %disp(cMed)
             %disp(timestep)
             %pause()
...
%         
        if timestep-dtauStep >= dtau
            realDtau = timestep-dtauStep;
            nCell = nCell + realDtau*DCell/(dz^2)*([nCell(1,:); nCell(1:Nz-1,:)] - 2*nCell + [nCell(2:Nz,:); nCell(Nz,:)]);
        ...
        if timestep-reffStep >= dtau
            realReff = ones(Nz,1)*r0' + (1/kSat*cMed)*((rint<0).*rint)' + (cMed./(cMed + kSat))*((rint>=0).*rint)';
            %disp(realReff)
            %pause()
            realDtau2 = timestep-reffStep;
            
            %nCell = nCell + realDtau2*(realReff.*nCell);
            nCell = nCell + realDtau2*(realReff.*(ones(Nz,Nc)-1/kY*sum(nCell,2)*ones(1,Nc)).*nCell);
            nCell(nCell < ExtTh) = 0;
